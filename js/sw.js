...
createRestaurantHTML = (restaurant,reviews,webpsrcset, jpgsrcset, imagetext,imageurl,urltext,url) => {
  ...
  li.className += ' restaurant__container--active';
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_idb__ = __webpack_require__(6);
...
const dbPromise = __WEBPACK_IMPORTED_MODULE_0_idb___default.a.open('mwsreviews',1,function(upgradeDb){
...
document.querySelector('.restaurantdetails__reviewform').addEventListener('submit',function(e){
  ...
  .catch(function(){
    // Whoops, something went wrong lets see if we can try this again later
    if (navigator.serviceWorker) {
      navigator.serviceWorker.ready.then(registration => {
        // Put the review into IndexedDB for later
        return dbPromise.then(function(db){
          const tx = db.transaction('reviews', 'readwrite');
          tx.objectStore('review').put(item);
          return tx.complete;
        })
        .then(function(){
        // Register a sync with the ServiceWorker
          return registration.sync.register('add-review');
        })
      })
    }
  })
